...
ecuador_stats <- tibble(
  mean = avg_ecuador_rating,
  sd = sd_ecuador_rating,
  total = num_ecuador_reviews,
)
...
boxplots_choc_ratings <- subset_atleast_50_reviews %>%
  ggplot(aes(x = percent_chocolate, y = rating, fill = country_of_bean_origin)) +
  geom_boxplot() +
  facet_wrap(~ country_of_bean_origin) +
  scale_fill_manual(values = paletteer_d("beyonce::X117")) +
...
# Look at table of average ratings by cocoa percent range overall for comparison
...
choc_violin_plot <- chocolate_w_continent %>%
  ...
  geom_violin() +
  scale_fill_manual(values = paletteer_d("ltc::fernande")) +
...
# Re-order factor levels so that ingredients are first, followed by characteristics instead of alphabetical order so that they are arranged in a more logical manner in the facets
long_choc_ingre_char_by_year <- long_choc_ingre_char_by_year %>%
  mutate(feature = fct_relevel(feature, "beans", "sugar", "cocoa_butter", "vanilla", "letchin", "salt",
                              "char_cocoa", "char_sweet", "char_nutty", "char_creamy", "char_roasty", "char_earthy"))

# Create scatter plots of mean scores of chocolate features by year with a linear trend line for each feature
scatterplot_choc_by_year <- long_choc_ingre_char_by_year %>%
  ggplot(aes(x = review_date, y = mean_score)) +
  geom_point(aes(color = feature)) +
  geom_smooth(method = "loess", se = FALSE, color = "black", linewidth = 0.3) +
  labs(title = "Scatterplots of Mean Proportion of Features Included in Reviewed Chocolate by Year",
       subtitle = "Most chocolate features remained relatively stable over time. There was little change in noted characteristics except for a slight decrease in reported char_creamy, and a u-shaped trend in char_cocoa. Possible corresponding changes in ingredient usage could be that of lecithin and vanillla decreasing, and cocoa_butter having a slight u-shaped trend, respectively.",
       x = "Year of Review",
       y = "Mean Proportion of Ingredients or Characteristics in Chocolate",
       caption = "Mia Silva") +
  facet_wrap(~ feature, ncol = 4, dir = "v") +
  theme_bw() +
  theme(plot.title = element_text(size = 14, face = "bold", color = "black", hjust = 0.5),
        axis.text.x = element_text(angle = 45, hjust = 1),
        plot.subtitle = element_textbox_simple(size = 10, hjust = 0.5, margin = margin(b = 10, unit = "pt")),
        legend.position = "none") +
  scale_colour_manual(values = paletteer_d("tidyquant::tq_light"))

# Display the scatterplots of mean scores of chocolate features by year
scatterplot_choc_by_year
```

NOTE: the mean_score variable is rather ambiguous in this context, as it translates to the mean proportion of reviews that included a certain ingredient or characteristic. Thus, I made the title and y-axis label more descriptive to clarify this. Mean proportion corresponds to mean score.
...
-   I made the plot background dark gray, which is both jarring to look at, especially when maintaining a light theme otherwise. There may be other scenarios where a dark background is useful to create contrast, or is aesthetically preferable, but consistency is key and this particular example simply makes the data harder to look at.
...
-  I rotated the x-axis text 90 degrees, which makes it harder to read. This could be useful if you have a lot of labels that would overlap if they were horizontal, but in this case it is unnecessary. Rotating at a different angle could also help with space and legibility when each label has a longer character length.
...
chocolate %>%
  ...
  labs(title = "Violin Plot of Ratings of All Chocolate by Year",
       subtitle = "This plot shows that overall ratings of chocolate improved over the years, with the range of ratings each year getting smaller and clustering to be consistently close to and above 3.0. This indicates more agreemennt amongst reviewers, suggesting consistency in chocolate quality. You can see more distinct patterns of rating distribution clustered at certain values in years 2007, 2009, and 2017.",
       x = "Year of Review",
       y = "Chocolate Ratings") +
  theme(plot.title = element_text(size = 14, face = "bold", color = "black", hjust = 0.5),
        axis.text.x = element_text(angle = 45, hjust = 1),
...
-   I also got rid of the legend that was off to the side. Since "review_date" was already displayed as "Year of Review" on the x-axis, it was redundant and simply cluttered the plot.
